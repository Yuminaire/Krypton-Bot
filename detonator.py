...
noName = discord.Embed(title='No Name Provided', description='Please provide a username after the command, or link your Discord user ID with your Minecraft name with the command **link**.', color=11141120
                       ).set_footer(text='Detonator Bot made by Yuushi#5964')
...
async def remindLink():
    ...
    await unlink_channel.send(embed=discord.Embed(title='Link your Account', description='Please make sure to link your Minecraft username with your Discord account so that your guild role can be updated automatically. You can do so by going into the <#714643693738655804> channel and typing `$link <ign>`.',
                                                  color=11141120).set_footer(text='Detonator Bot made by Yuushi#5964'))
...
async def setRoles():
    ...
    for member in guild.members:
        if dataDB.get(Query()['userID'] == member.id):
            ...
            if 0 <= weeklyEXP < 125000:
                rank = 'Trainee'
            elif 125000 <= weeklyEXP <= 175000:
                rank = 'Bomber'
            else:
                rank = 'Detonator'
            ...
            if role not in member.roles:
                ...
                await bot_channel.send('<@' + str(member.id) + '>', embed=discord.Embed(title=':bomb: C4 Exploded!', description='Your rank has been changed to **' + rank + '** because of the total of Guild Experience you earned this week. \n **Total Weekly GEXP**: ' + format(weeklyEXP,
                                                                                                                                                                                                                 ',d') + ' exp',
                                                           color=43520).set_footer(text='Detonator Bot made by Yuushi#5964'))


async def delete_role(member, rank, guild):
    if rank == 'Detonator':
        await member.remove_roles(discord.utils.get(guild.roles, name='Trainee'), discord.utils.get(guild.roles, name='Bomber'))
    elif rank == 'Bomber':
        await member.remove_roles(discord.utils.get(guild.roles, name='Trainee'), discord.utils.get(guild.roles, name='Detonator'))
    elif rank == 'Trainee':
        await member.remove_roles(discord.utils.get(guild.roles, name='Bomber'), discord.utils.get(guild.roles, name='Detonator'))
...
async def help(ctx):
    ...
    helpMessage.set_footer(text='Detonator Bot made by Yuushi#5964')
...
async def link(ctx, name=None):
    if name is None:
        ...
        if uuid in uuidList:
            ...
            traineeRole = discord.utils.get(guild.roles, name='Trainee')
            ...
            await member.add_roles(traineeRole)
            await member.add_roles(linkRole)
        else:
            await ctx.send(embed=discord.Embed(title='Impersonator!',
                                               description='You are not a member of the Detonator\'s guild so you can\'t link your Minecraft name to the guild\'s bot. Don\'t try to impersonate us!',
                                               color=11141120
                                               ).set_footer(text='Detonator Bot made by Yuushi#5964'))
...
async def list(ctx):
    ...
    if (ctx.message.author.id == 272553041826414593) or (ctx.message.author.id == 193425484799803395):
        ...
        await ctx.send(embed=discord.Embed(title='An error has occured', description='This command is reserved to the guild owner <@' + str(272553041826414593) + '> and to the developer of the bot <@' + str(193425484799803395) + '>.', color=11141120))
